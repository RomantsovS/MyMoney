					...
					Если Структ.ИмяТипаМетаданных = "Константы" Тогда 
						...
						Если ОбщегоНазначения.ЭтоСсылка(ПервыйТип) Тогда 
							Если ЗначениеЗаполнено(Структ.Value) Тогда 
								...
								//	ЗначениеСсылка = Справочники[Строка(ПервыйТип)].ПолучитьСсылку(Новый УникальныйИдентификатор(Структ.Value));
								//ИначеЕсли Перечисления.ТипВсеСсылки().СодержитТип(ПервыйТип) Тогда 
								//	ЗначениеСсылка = Перечисления[Строка(ПервыйТип)].ПолучитьСсылку(Новый УникальныйИдентификатор(Структ.Value));
								//КонецЕсли;
