...
Функция ПолучениеСМССообщения(МассивТелефонов, Период = Неопределено) Экспорт 
	...
	#Иначе
		...
		СтруктураСообщения = Новый Структура("ДатаПолучения, НомераТелефонов, Текст");
		СтруктураСообщения.ДатаПолучения = ТекущаяДата();
		МассивНомеров = Новый Массив;
		МассивНомеров.Добавить("Tinkoff");
		СтруктураСообщения.НомераТелефонов = МассивНомеров;
		СтруктураСообщения.Текст = "Покупка. Карта *0754. 384 RUB. MYASNOV. Доступно 32678.93 RUB";	
		МассивСообщений.Добавить(СтруктураСообщения);
		
		СтруктураСообщения = Новый Структура("ДатаПолучения, НомераТелефонов, Текст");
		СтруктураСообщения.ДатаПолучения = Дата(2018, 6, 5, 1, 1, 1);	
		МассивНомеров = Новый Массив;
		МассивНомеров.Добавить("900");
		СтруктураСообщения.НомераТелефонов = МассивНомеров;
		СтруктураСообщения.Текст = "Перевод 4960р от Андрей Михайлович К. Баланс ECMC6285: 15000р";
		МассивСообщений.Добавить(СтруктураСообщения);
		...
		МассивНомеров.Добавить("900");
		СтруктураСообщения.НомераТелефонов = МассивНомеров;
		СтруктураСообщения.Текст = "ECMC6285 05.06.18 12:44 Оплата 1992.60р Баланс 31899.56р";
		МассивСообщений.Добавить(СтруктураСообщения);
