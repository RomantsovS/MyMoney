...
Функция НастройкиЗагрузитьПоОбъекту(КлючОбъекта) Экспорт 
	...
	Для Каждого СтрокаНабора Из НаборЗ Цикл
		СписокРезультат.Добавить(СтрокаНабора.Настройки, СтрокаНабора.КлючНастроек);
	КонецЦикла;
...
Процедура СохранитьНастройкуОтчетаПриЗакрытии(Форма) Экспорт 
	...
	ОбщиеНастройкиСохранитьНастройкуОтчетаПриЗакрытии(Форма.КлючОбъекта, СтруктураНастроек);
...
Процедура ВыбратьНастройкуЗавершениеСервер(Форма) Экспорт 
	...
	Настройка = ОбщиеНастройкиЗагрузить(Форма.КлючОбъекта, Форма.ТекНастройка);
