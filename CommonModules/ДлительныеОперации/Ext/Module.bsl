...
Функция ЗаданиеВыполнено(Знач ИдентификаторЗадания) Экспорт
	...
	Если Задание = Неопределено Тогда
		Сообщить(НСтр("ru = 'Длительные операции.Фоновое задание не найдено'"));
	Иначе
		...
		ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
			Сообщить(НСтр("ru = 'Длительные операции.Фоновое задание отменено администратором'"));
		Иначе
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтменитьВыполнениеЗадания(Знач ИдентификаторЗадания) Экспорт 
	...
	Исключение
		...
		Сообщение.Текст = НСтр("ru='Длительные операции.Отмена выполнения фонового задания'");
		Сообщение.Сообщить();
	КонецПопытки;
...
Процедура ВывестиСообщенияЗадания(ИдентификаторЗадания, ИдентификаторНазначения = Неопределено) Экспорт 
	...
	Для Каждого Сообщ Из Сообщения Цикл
		Если ИдентификаторНазначения <> Неопределено Тогда 
			Сообщ.ИдентификаторНазначения = ИдентификаторНазначения;
		КонецЕсли;
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Задание = НайтиЗаданиеПоИдентификатору(ИдентификаторЗадания);
	Если Задание = Неопределено Тогда
		Сообщить(НСтр("ru = 'Фоновое задание не найдено:'") + " " + Строка(ИдентификаторЗадания));
		...
		Если ЧитатьПрогресс И СтрНачинаетсяС(Сообщение.Текст, "{") Тогда
			...
			Если Позиция > 2 Тогда
				...
				Если ИдентификаторМеханИзма = СообщениеПрогресса() Тогда
					...
					Результат.Прогресс = новый Структура("Текст", ПолученныйТекст);
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
