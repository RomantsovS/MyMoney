...
Процедура ПередЗавершениемРаботыСистемы(Отказ = Ложь, ТекстПредупреждения = "") Экспорт
	...
	БекапЧерезПочтуКлиент.ПередЗавершениемРаботыСистемы(Отказ, Предупреждения);
...
Процедура ЗаполнитьПараметрыКлиента(ПараметрыКлиента) Экспорт
	...
	Если ТипЗнч(ПараметрыПриложения[ИмяПараметра]) <> Тип("Структура") Тогда
		...
		ПараметрыПриложения[ИмяПараметра].Вставить("НеобходимаВыгрузкаБекапаНаПочту");
		ПараметрыПриложения[ИмяПараметра].Вставить("ТекстОтносительнойДатыБекапа");
	КонецЕсли;
