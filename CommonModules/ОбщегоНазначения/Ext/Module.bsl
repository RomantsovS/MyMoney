...
// с устаревшей процедурой СообщитьПользователю.
...
Процедура ПровестиВсеДокументы(Знач ПараметрыВыгрузки, АдресХранилища = Неопределено) Экспорт 
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}0%'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			ДокО.Записать(РежимЗаписиДокумента.Проведение);
		Исключение
			СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Возникла ошибка при проведении %1: %2'"),
			ДокО, ОписаниеОшибки()));
		КонецПопытки;
		
		СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура УдалитьВсеДанныеНаСервере(Знач ПараметрыВыгрузки, АдресХранилища = Неопределено) Экспорт 
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}3%'"));
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}4%'"));
	
	Для Каждого СтрМета Из Метаданные.Константы Цикл
		Константы[СтрМета.Имя].Установить(Неопределено);
	КонецЦикла;
	
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}5%'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}%1%'"),
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}91%'"));
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}92%'"));
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}93%'"));
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}94%'"));
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}95%'"));
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}98%'"));
	...
	СообщитьПользователю(НСтр("ru = '{СтандартныеПодсистемы.ДлительныеОперации}100%'"));
	...
	Для ТекущаяКолонка = 1 По ТабличныйДокумент.ШиринаТаблицы Цикл
		...
		Для ТекущаяСтрока = НачальнаяСтрока По КонечнаяСтрока Цикл
			...
			Если КоличествоУровнейГруппировокСтрок > 0 И ТекущаяСтрока = НачалоДанных Тогда
				...
				// Для первой строки с данными получим значение авто отступа
			...
			Для НомерСтрокиТекста = 1 По СтрЧислоСтрок(ТекстЯчейки) Цикл
				...
				// Если используется авто отступ, то прибавим к ширине ячейки его величину
