...
Процедура ВыгрузитьРезервнуюКопиюНаПочту(Знач ПараметрыВыгрузки, АдресХранилища = Неопределено) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выгрузка резервной копии на почту выполнена успешно'"));
...
Функция ПроверитьНеобходимостьВыгрузкиРезервнойКопииНаПочту(ТекстОтносительнойДатыРезервнойКопии = Неопределено) Экспорт 
	...
	ДатаПоследнегоДекапа = Константы.ДатаПоследнегоРезервногоКопирования.Получить();
	...
	Если ТекстОтносительнойДатыРезервнойКопии <> Неопределено Тогда 
		ТекстОтносительнойДатыРезервнойКопии = ОбщегоНазначенияКлиентСервер.ОтносительнаяДатаСинхронизации(ДатаПоследнегоДекапа);
	КонецЕсли;
