...
Процедура ПолучитьДанныеОтчета(СхемаКомпоновкиДанных, Результат, ПорядокИтогов, УникальныйИдентификатор,
	...
	Если АдресНастроекСКД = Неопределено Тогда 
			...
			Если СтрНайти(Настройка.Ключ, "Отбор") <> 0 Тогда 
				...
				Если ТипНастройки = Тип("СписокЗначений") Тогда 
					Если Настройка.Значение.Количество() > 0 Тогда 
						...
						УстановитьОтбор(Настройки, ИмяПоля, ВидСравненияКомпоновкиДанных.ВСписке,
		...
		Если ТипЗнч(Результат) <> Тип("ТабличныйДокумент") Тогда 
			УстановитьПараметрВывода(Настройки, "ВертикальноеРасположениеОбщихИтогов", РасположениеИтоговКомпоновкиДанных.Нет, Истина);
				...
				Для Каждого Стр из ПорядокИтогов Цикл			
					Если Стр.Использование = Истина Тогда
						Если Не ТекИтогОригинал Тогда 
							...
							СкопироватьГруппировку(НовГруппировка, ИтогВалюта);
		...
		Если СтруктураПараметров <> Неопределено Тогда 
			...
			//Если КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(Настройки, "Периодичность").Значение = Перечисления.Периодичность.Регистратор Тогда
				...
				Если ГруппировкаРегистратор <> Неопределено Тогда 
					НоваяСтрока = ГруппировкаРегистратор.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
					НоваяСтрока.Поле = Новый ПолеКомпоновкиДанных("Период");
...
Процедура ИнициализироватьНастройкиГруппировки(ЭлементСтруктуры, СтруктураКомпоновки) Экспорт 
	...
	Для Каждого ЭлементПолеГруппировки Из ЭлементСтруктуры.ПоляГруппировки.Элементы Цикл 
		НоваяСтрока.Поле = ЭлементПолеГруппировки.Поле;
		НоваяСтрока.Использование = ЭлементСтруктуры.Использование;
		НоваяСтрока.Имя = ЭлементСтруктуры.Имя;
	КонецЦикла;
...
Функция ПустаяТаблицаСтруктураКомпоновки() Экспорт 
	...
	СтруктураКомпоновки.Колонки.Добавить("Имя");

	Возврат СтруктураКомпоновки;
	
КонецФункции

Функция ПустаяТаблицаВыбранныеПоля() Экспорт 
	
	ВыбранныеПоля = Новый ТаблицаЗначений;
	ВыбранныеПоля.Колонки.Добавить("Поле", Новый ОписаниеТипов("Строка"));
	ВыбранныеПоля.Колонки.Добавить("Использование");
...
Процедура ОбработкаРасшифровкиСервер(Расшифровка, Парам, АдресДанныхРасшифровки, СхемаКомпоновкиДанных) Экспорт	
	ДанныеРасшифровкиОтчета = ДанныеРасшифровкиОтчета(Расшифровка, АдресДанныхРасшифровки);
...
Процедура НазадУровеньДиаграммы(Форма) Экспорт 
	...
	ОтборТекУровень = Форма.МассивОтборТекУровень.Получить(Форма.МассивОтборТекУровень.Количество() - 1);
	...
	Форма.МассивОтборТекУровень.Удалить(ОтборТекУровень);
...
Процедура ПриСозданииНаСервере(Форма) Экспорт
	...
	Иначе
		ОбщегоНазначения.СообщитьПользователю(СтрШаблон(НСтр("ru = 'Не удалось получить СКД Отчета: %1'"), ИмяМакетаСКД));
	КонецЕсли;
...
Функция ПрименитьИзмененияКСхемеКомпоновкиДанных(АдресСКД, АдресНастроекСКД, УникальныйИдентификатор) Экспорт
	...
	Если ОбщегоНазначения.ПолучитьXML(КомпоновщикНастроек.ПолучитьНастройки()) <>
		ОбщегоНазначения.ПолучитьXML(ПолучитьИзВременногоХранилища(АдресНастроекСКД)) Тогда
...
Функция СписокДоступныхПараметровУсловногоОформления() Экспорт 
	...
	СписокПараметров.Добавить("МаксимальнаяШирина", "Макс. ширина");
...
Функция ПолучитьПредставлениеПараметраУсловногоОформления(Параметр) Экспорт 
	...
	СписокПараметров = СписокДоступныхПараметровУсловногоОформления();
	
	ЭлементСписка = СписокПараметров.НайтиПоЗначению(Параметр);
...
Функция ПолучитьГруппировкуРекурсивно(ЭлементСтруктуры, Поле)
	...
	Если ЭлементСтруктуры.Структура.Количество() > 0 Тогда 
		Для Каждого ЭлемСтруктурыВнутр Из ЭлементСтруктуры.Структура Цикл 
			Возврат ПолучитьГруппировкуРекурсивно(ЭлемСтруктурыВнутр, Поле);
		КонецЦикла;
	КонецЕсли;
