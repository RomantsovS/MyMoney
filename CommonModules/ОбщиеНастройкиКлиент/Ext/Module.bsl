...
Процедура СохранитьНастройку(Форма) Экспорт 
	...
	Если ЗначениеЗаполнено(Форма.ТекНастройка) Тогда 
		...
		Если ОбщиеНастройкиВызовСервера.СохранитьНастройкуНаСервереПроверкаТекНастройки(Форма.ТекНастройка) Тогда 
			СписокВариантов.Добавить(Форма.ТекНастройка, "Перезаписать """ + Форма.ТекНастройка + """");
		КонецЕсли;
		
		СписокВариантов.Добавить(ПредопределенноеЗначение("Справочник.КлючНастроек.ПустаяСсылка"), НСтр("ru = 'Создать новую'"));
		
		ОткрытьФорму("ОбщаяФорма.ФормаВыбораИзСписка", Новый Структура("Список", СписокВариантов), Форма, , , ,
