//Процедура ПриНачалеРаботыСистемы(Оповещение, Форма = Неопределено) Экспорт 
//	Если БекапЧерезПочту.ИспользоватьБекапНаПочту() Тогда 
//		ТекстОтносительнойДатыБекапа = "";
//		
//		Если БекапЧерезПочту.ПроверитьНеобходимостьВыгрузкиБекапаНаПочту(ТекстОтносительнойДатыБекапа) Тогда 
//			ПоказатьВопрос(Новый ОписаниеОповещения("ВопросВыгрузкиЕмейлБекапаПриНачалеРаботыСистемыЗавершение", БекапЧерезПочтуКлиент, 
//			Новый Структура("ОбработкаПродолжения, Форма", Оповещение, Форма)),
