...
Процедура Изменить(Команда)
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		...
		Если Не ПраваВыполненияКода.ДоступноВыполнениеКодаВНебезопасномРежиме И Объект.РежимВыполнения = 1 Тогда
			Объект.РежимВыполнения = 0; // безопасный режим
		КонецЕсли;
...
Функция ДоступныеПраваВыполнениеКода()
	...
	Результат.Вставить("ДоступноВыполнениеКодаВНебезопасномРежиме", Истина);
	Результат.Вставить("ДоступноВыполнениеКода", Истина);
...
Процедура ИзменитьОбъекты()
	...
	Если Объект.ИзменятьВТранзакции Тогда
		...
		Если ТОНастройкаПорции = 1 Тогда // обработка одним вызовом
			...
			ПоказатьПредупреждение(, НСтр("ru = 'Пожалуйста подождите, обработка может занять некоторое время...'"), 1, НСтр("ru = 'Изменение выделенных элементов'"));
	...
	Иначе
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Обрабатываются элементы...'"), , НСтр("ru = 'Изменение выделенных элементов'"));
...
Процедура ОбработатьРезультатИзменения(РезультатИзменения = Неопределено, ПродолжитьОбработку = Неопределено)
	...
	Если ТекущееСостояниеИзменения.ПоказыватьПроцентОбработанных Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Обрабатываются элементы...'"), ТекущийПроцент, НСтр("ru = 'Изменение выделенных элементов'"));
	КонецЕсли;
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		...
		НСтр("ru = 'Изменение реквизитов элементов'"));
...
Процедура ДобавитьСвойствоВСписокРеквизитов(Свойство) 
	// ++ 23.08.2018 13:16 Кремнёва EEYagafarova@1cbit.ru RKERP-763 / 
	//ОписаниеСвойства = ЗначенияРеквизитовОбъекта(Свойство, "Ссылка,Наименование,ТипЗначения,ЭтоДополнительноеСведение");
	ОписаниеСвойства = ЗначенияРеквизитовОбъекта(Свойство, "Ссылка,Наименование,ТипЗначения,ЭтоДополнительноеСведение,ркРазрешитьГрупповоеИзменение");
	Если КонтекстныйВызов И Не ОписаниеСвойства.ркРазрешитьГрупповоеИзменение Тогда
		Возврат;
	КонецЕсли; 
	// -- 23.08.2018 13:16 Кремнёва EEYagafarova@1cbit.ru RKERP-763 / 
