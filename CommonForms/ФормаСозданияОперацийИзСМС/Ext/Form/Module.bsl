...
Процедура ОбновитьСписокОперацийOFX()
	...
	Если ТипЗнч(ДанныеФайла.BANKMSGSRSV1.STMTTRNRS) = Тип("ОбъектXDTO") Тогда 
		Для Каждого ОперацияФайла Из ДанныеФайла.BANKMSGSRSV1.STMTTRNRS.STMTRS.BANKTRANLIST.STMTTRN Цикл 
			ПрочитатьОперациюOFX(ТЗ, Кошелек, ОперацияФайла);
		КонецЦикла;
	ИначеЕсли ТипЗнч(ДанныеФайла.BANKMSGSRSV1.STMTTRNRS) = Тип("СписокXDTO") Тогда 
		Для Каждого ПачкаОпераций Из ДанныеФайла.BANKMSGSRSV1.STMTTRNRS Цикл 
			Для Каждого ОперацияФайла Из ПачкаОпераций.STMTRS.BANKTRANLIST.STMTTRN Цикл 
				ПрочитатьОперациюOFX(ТЗ, Кошелек, ОперацияФайла);
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура ПрочитатьОперациюOFX(ТЗ, Кошелек, ОперацияФайла)
