...
Процедура ОбновитьСписокОпераций()
	...
	МассивТелефонов = ПолучитьШаблоныСМСНомераТелефонов();
	...
	Для Каждого СМССообщение Из МассивСМС Цикл 
		...
		//ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru = ':%1, :%2, :%3'"),
		//ДатаПолучения, СМССообщение.НомераТелефонов[0], СМССообщение.Текст));		
...
Процедура ОбновитьСписокОперацийСервер(МассивДанныхСМС)
	...
	Для Каждого Стр Из МассивДанныхСМС Цикл 
		...
		Если Рез.НайтиСледующий(СтруктураПоиска) Тогда 			
			НовСтр = ТЗ.Добавить();
...
Функция ПолучитьШаблоныСМСНомераТелефонов()
	
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ШаблоныЧтенияСМС.НомерТелефона
	|ИЗ
