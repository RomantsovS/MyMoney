...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ОбновитьОстатки" Тогда 
		ЗаполнитьОстатки();
	ИначеЕсли ИмяСобытия = "ЗагруженаРезервнаяКопия" Тогда 
		ОбновитьБД();
	КонецЕсли;
КонецПроцедуры
...
Процедура Подключаемый_ПроверитьВыполнениеЗадания()
	...
	Попытка
		Если ЗаданиеВыполнено(ИдентификаторЗадания) Тогда
			ДлительныеОперацииВызовСервера.ВывестиСообщенияЗадания(ИдентификаторЗадания, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ПослеВыгрузкиРезервнойКопииНаПочту(Результат, ДопПараметры) Экспорт 
	ТекущаяОперация = "ЗагрузкаКурсовВалют";
	ПодключитьОбработчикОжидания("Подключаемый_ЗапуститьПродолжитьОчередьПроцедурПослеОткрытия", 0.1, Истина);	
КонецПроцедуры
...
Процедура ЗаполнитьОстатки()
	...
	Пока Рез.Следующий() Цикл
		...
		НоваяСтрока.ВалютаКраткоеНаименование = Рез.ВалютаКраткоеНаименование;
		НоваяСтрока.Сумма = Рез.сумма;
		НоваяСтрока.КошелекКод = "Итого:";
		НоваяСтрока.СтрокаИтог = Истина;
		
		РезКош = Рез.Выбрать();
		
		Пока РезКош.Следующий() Цикл
			НоваяСтрока = Остатки.Добавить();
			НоваяСтрока.ВалютаКраткоеНаименование = РезКош.ВалютаКраткоеНаименование;
			НоваяСтрока.Сумма = РезКош.Сумма;
			НоваяСтрока.КошелекКод = РезКош.КошелекКод;			
			НоваяСтрока.Кошелек = РезКош.Кошелек;
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
